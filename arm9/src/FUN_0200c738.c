/*
 * FUN_0200c738.c — auto-generated stub by tools/scripts/auto_decomp_leaf.py
 *
 * Source: arm9.bin @ 0x0200c738 (load base 0x02000000).
 * Raw ARM instruction words (little-endian):
 *
 *   0200c738  e12fff1e
 *
 * This translation unit exists so the fnptr-table generator picks up
 * a host-side symbol named "FUN_0200c738".  When the underlying function
 * becomes hot enough to matter, replace this body with a hand-written
 * decompilation; until then it returns the AAPCS-compatible default.
 */

#include "types.h"
#include <stdint.h>

void FUN_0200c738(int param_1, int param_2)
{
    /* immediate return */
}
