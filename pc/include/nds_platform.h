...
/* === ARM9 main RAM ===
 * VirtualAlloc 4 MiB anchored at NDS address 0x02000000 and memcpy
 * extracted/arm9.bin into it at offset 0x4000.  Once mapped, decompiled
 * code that reads through raw pointer literals like
 *   *(volatile uint32_t *)0x020055B4
 * sees the real .data initial values from the user's ROM dump.
 *
 * Returns the host base pointer (== 0x02000000 cast to void*) on success
 * or NULL if the fixed-address mapping was unavailable.
 */
void  nds_arm9_ram_init(void);
void *nds_arm9_ram_base(void);
int   nds_arm9_ram_is_mapped(void);

/* Apply ROM-extracted .data values into every C-level DAT_<addr> global.
 * Called once at startup, after nds_arm9_ram_init().  Generated by
 * tools/scripts/gen_dat_init_from_stubs.py at configure time. */
void  nds_apply_dat_inits(void);
